...
## Packaging With ThunderPipe.Sdk

The [PEAK BepInEx Template](<https://github.com/PEAKModding/BepInExTemplate>) comes with Thunderstore packaging built-in, using [ThunderPipe](<https://github.com/WarperSan/ThunderPipe>).
...
Once you have the ZIP package and have confirmed that it looks correct, you can jump straight into [Uploading to Thunderstore](#uploading-to-thunderstore)!
...
### Uploading with ThunderPipe.Sdk

ThunderPipe.Sdk can also publish your package. This can be done easily by adding `-p:PublishTS=true` to the build command for your project if you used the template:

```sh
dotnet build -c Release -v d -p:PublishTS=true
```

:::note
Publishing to Thunderstore via ThunderPipe.Sdk requires that you've set up your Thunderstore token for your Thunderstore team on your machine. See the `Config.Build.user.props` file in your project for setup instructions!
:::
